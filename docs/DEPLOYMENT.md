...
3. From the prompt, load the schema and feature migrations (in order):
   ...
   `006`/`007`/`008` are required for the favourites, hotel map, and per-room name/special-amenity features. `003_storage.sql` runs **inside Supabase**, not Postgres — see Step 2 above. `005_seed_images.sql` is regenerated by `scripts/upload-images.js`; apply it only if you also loaded `004_seed.sql`.

4. **Skip** `004_seed.sql` (and the matching `005_seed_images.sql`) in production unless you specifically want the sample dataset (50 dummy customers, 8 hotels, etc. — and remember the seed password is the public `password123`).
